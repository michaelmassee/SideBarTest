...
public abstract class PanelBase implements XToolPanel, XWindowListener, XSidebarPanel, XComponent {
	...
	protected PanelBase() {
		maDisposeListeners = new Vector<>();
	}

	protected void Initialize(final XWindow xParentWindow, final XComponentContext xContext) {
		...
		try {
			...
			if (xParentPeer != null) {
				...
				XToolkit xToolkit = UnoRuntime.queryInterface(XToolkit.class, xFactory.createInstanceWithContext("com.sun.star.awt.Toolkit", xContext));
				WindowDescriptor aWindowDescriptor = new WindowDescriptor(WindowClass.CONTAINER, "", xParentPeer, (short) -1, // parent index not available
						new Rectangle(0, 0, 10, 10), WindowAttribute.SIZEABLE | WindowAttribute.MOVEABLE | WindowAttribute.NODECORATION);
				mxWindow = UnoRuntime.queryInterface(XWindow.class, xToolkit.createWindow(aWindowDescriptor));
				if (mxWindow == null)
				...
				final XWindowPeer xPeer = UnoRuntime.queryInterface(XWindowPeer.class, mxWindow);
				if (xPeer != null) {
