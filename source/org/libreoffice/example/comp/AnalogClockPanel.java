...
/**
 * Implementation of a sidebar panel that shows an analog clock. The focus of this class lies on showing how to implement a sidebar panel that uses a canvas to display its content.
 */
public class AnalogClockPanel extends PanelBase {
	AnalogClockPanel(final XWindow xParentWindow, final XComponentContext xContext, final XCanvas xCanvas) {
		mxCanvas = xCanvas;
		mnCurrentSecond = -1;
		...
		// almost two seconds.
		maTimer = new Timer();
		maTimer.scheduleAtFixedRate(new TimerTask() {
			@Override
			public void run() {
	...
	private void PaintCurrentTime() {
		...
		try {
			...
			DrawShape(new Ellipse2D.Double(nCenterX - nRadius, nCenterY - nRadius, 2 * nRadius, 2 * nRadius));
	 ...
	 *
	 * @param nValueType One of Calendar.SECOND, Calendar.MINUTE, Calendar.HOUR defines the range of nValue. It is [0,60) for seconds and minutes and [0,12) for hours.
	 */
	private void PaintHand(final double nCenterX, final double nCenterY, final double nRadius, final Color aColor, final double nValue, final int nValueType) {
		...
		try {
			DrawShape(new Line2D.Double(nCenterX, nCenterY, nCenterX + Math.sin(nAngle) * nLocalRadius, nCenterY + Math.cos(nAngle) * nLocalRadius));
		} catch (IllegalArgumentException e) {
			e.printStackTrace();
		}
	}
	 ...
	 */
	private void SetColor(final RenderState aRenderState, final Color aColor) {
		aRenderState.DeviceColor = new double[] { aColor.getRed() / 255.0, aColor.getGreen() / 255.0, aColor.getBlue() / 255.0, aColor.getAlpha() / 255.0 };
	}

	/**
	 * Callback from the sidebar layouter. The returned size structure states that - the minimum height is a fixed value - the maximum height is at least the minimum size or,
	 * preferably, the width of the panel. - the preferred height is the same as the maximum height.
	 */
	@Override
	...
	/**
	 * Convert a Java Shape object into a series of canvas draw commands. This takes advantage of the fact that every Java Swing shape can be converted into a series of outline
	 * parts such as lines and bezier curves. This allows us to paint every Java shape while only implementing a few outline primitives.
	 */
	private void DrawShape(final Shape aShape) throws IllegalArgumentException {
		...
		while (!aPathIterator.isDone()) {
			...
			case PathIterator.SEG_CUBICTO:
				...
				mxCanvas.drawBezier(new RealBezierSegment2D(nX, nY, aCoordinates[0], aCoordinates[1], aCoordinates[4], aCoordinates[5]),
						new RealPoint2D(aCoordinates[2], aCoordinates[3]), maViewState, maRenderState);
				nX = aCoordinates[4];
				nY = aCoordinates[5];
				break;

			case PathIterator.SEG_LINETO:
				mxCanvas.drawLine(new RealPoint2D(nX, nY), new RealPoint2D(aCoordinates[0], aCoordinates[1]), maViewState, maRenderState);
				nX = aCoordinates[0];
				nY = aCoordinates[1];
				break;
			...
			case PathIterator.SEG_QUADTO:
				mxCanvas.drawBezier(new RealBezierSegment2D(nX, nY, aCoordinates[0], aCoordinates[1], aCoordinates[0], aCoordinates[1]),
						new RealPoint2D(aCoordinates[2], aCoordinates[3]), maViewState, maRenderState);
				nX = aCoordinates[2];
				nY = aCoordinates[3];
				break;
	...
	public static void SetColor(final ColorType eColorType, final Color aColor) {
		switch (eColorType) {
		default:
		case FaceColor:
			maFaceColor = aColor;
